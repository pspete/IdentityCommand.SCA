function Merge-SCAParameter {
    <#
    .SYNOPSIS
    Projects supplied parameters onto an ordered request template.

    .DESCRIPTION
    Several SCA commands build a request body by walking a fixed [ordered] template of expected
    ...
    order and is ready to pass to ConvertTo-SCAJsonBody.
    ...
    .PARAMETER Fallback
    Optional object (e.g. the existing policy from Get-SCAPolicy). When supplied, an unbound key takes
    its value from this object's matching property instead of the template default.

    .EXAMPLE
    $Properties = Merge-SCAParameter -Template $OrderedProperties -BoundParameter ($PSBoundParameters | Get-Parameter)

    .EXAMPLE
    $Properties = Merge-SCAParameter -Template $OrderedProperties -BoundParameter $boundParameters -Fallback $CurrentPolicy
    #>
